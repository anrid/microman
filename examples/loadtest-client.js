...
/*
  ...
  ./examples/loadtest-client.js --host api-dev.taskworld.com --port 11100

  Multiple concurrent clients. Same as above but with 10 concurrent clients.
  ./examples/loadtest-client.js --host api-dev.taskworld.com --port 11100 --clients 10

  Max throughput. Have 500 concurrent clients send 3 rapid 'echo's each with a delay of 500ms.
  ./examples/loadtest-client.js --host api-dev.taskworld.com --port 11100 --clients 500 --iterations 3 --delay 500

  Overload. Do heavy work with 10 concurrent clients causing API throttling to kick in.
  ./examples/loadtest-client.js --host api-dev.taskworld.com --port 11100 --clients 10 --iterations 10 --delay 100 --topic heavy
*/
'use strict'
...
function createClient (id, { host, port, topic, iterations, delay, clients }) {
  return new Promise(resolve => {
    ...
    if (clients < 10) log(`id=${id} event=connecting msg='Connecting to url: ${url}'`)
    ...
    ws.on('open', async () => {
      if (clients < 10) log(`id=${id} event=connected`)
      ...
      setTimeout(() => {
        log(`id=${id} event=done stats='${JSON.stringify(stats)}'`)
        ws.close()
      }, 3000)
    })
    ...
    ws.on('message', (data, flags) => {
      ...
      if (clients === 1 && iterations === 1) {
        log(`id=${id} Message: ${JSON.stringify(message, null, 2)}`)
      }

      if (clients < 10 && stats.messages % 100 === 0) {
        log(`id=${id} stats='${JSON.stringify(stats)}'`)
      }
    })

    ws.on('close', (code, message) => {
      if (clients < 10) log(`id=${id} event=disconnected code=${code}`)
      if (code !== 0) {
        reconnect()
      } else {
        resolve()
