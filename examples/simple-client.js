...
function createClient ({ host, port }) {
  const c = {
    ...
    send ({ topic, payload, waitFor }) {
      ...
      log(`id=${c.id} req=${requestId} send=1 topic=${topic}`)
      ...
      .timeout(3000)
      .catch(err => {
        log(`id=${c.id} req=${requestId} timeout=1`)
        throw err
      })
      .finally(() => {
        log(`id=${c.id} req=${requestId} clear=1`)
    ...
    connect () {
      return new P(resolve => {
        ...
        log(`id=${c.id} msg='connecting to url: ${c.url}'`)
        c.isConnecting = true
        c.isConnected = false
        ...
        c.ws.on('error', err => {
          log(`id=${c.id} error='${err.message}'`)
        })

        c.ws.on('message', data => {
          const message = JSON.parse(data)
          log(`id=${c.id} req=${message.meta.requestId} recv=1 topic=${message.topic}`)
